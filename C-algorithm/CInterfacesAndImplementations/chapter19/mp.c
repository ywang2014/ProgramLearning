...
T MP_mul(T z, T x, T y){
	// checked runtime errors for binary funcitions
	assert(x && y && z);
...
T MP_neg(T z, T x){
	...
	// checked runtime errors for unary funcitions
	assert(x && z);
...
T MP_modui(T x, unsigned long y){
	assert(x);
...
T MP_lshitf(T z, T x, int s){
	shitf(0, XP_lshift);
}


T MP_rshitf(T z, T x, int s){
	shitf(0, XP_rshift);
}


T MP_ashitf(T z, T x, int s){
	shitf(sign(x), XP_rshift);
}
...
char *MP_tostr(char *str, int size, int base, T x){
	...
	if (str == NULL){
		// size <- number of characters to represent x in base 
		int k;
		for (k = 5; (1 << k) > base; k--){
			;
		}
...
T MP_fmtu(int code, va_list *app, int put(int c, void *c1), void *c1,
...
void MP_fmt(int code, va_list *app, int put(int c, void *c1), void *c1,
	unsigned char flags[], int width, int precision){
	T x;
	int base;
	int size;
