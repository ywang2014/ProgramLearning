/**
	Multiple Precision interface
	
	49个函数，2个异常
	导出了n位有符号及无符号整数的，一组完整算法函数
*/
#ifndef MP_INCLUDED
#define MP_INCLUDED
...
extern T MP_lshift(T z, T x, T y);
extern T MP_rshift(T z, T x, T y);
extern T MP_ashift(T z, T x, T y);

extern T MP_fromstr(T z, const char *str, int base, char **end);
extern char *MP_tostr(char *str, int size, int base, T x);
extern void MP_fmt(int code, va_list *app, int put(int c, void *c1), void *c1,
	unsigned char flags[], int width, int precision);
extern void MP_fmtu(int code, va_list *app, int put(int c, void *c1), void *c1,
	unsigned char flags[], int width, int precision);
