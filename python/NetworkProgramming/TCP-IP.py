...
# TCP/IP 
	...
	# 互联网协议簇（Internet Protocol Suite）就是通用协议标准：把全世界的所有不同类型的计算机都连接起来
	...
	# 通信的时候，双方必须知道对方的标识，好比发邮件必须知道对方的邮件地址
		...
		# 一台计算机同时接入到两个或更多的网络，比如路由器，它就会有两个或多个IP地址，所以，IP地址对应的实际上是计算机的网络接口，通常是网卡。
	...
	# TCP协议
		# TCP协议则是建立在IP协议之上的
		# TCP协议负责在两台计算机之间建立“可靠连接”，保证数据包按顺序到达
		# TCP协议会通过(three times)握手建立连接，然后，对每个IP包编号，确保对方按顺序收到，如果包丢掉了，就自动重发
