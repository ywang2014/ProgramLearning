...
# UDP协议传输数据: 和TCP类似，使用UDP的通信双方也分为客户端和服务器。服务器首先需要绑定端口
s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
# 绑定端口号
s.bind(('127.0.0.1', 9999))	# bind()参数一个，类型为tuple
...
while True:
	...
	s.sendto((b'Helo, %s!', % data), addr)	# 等3.5出来就支持b'%s'了??
	# s.sendto(b'Helo, %s!' + data, addr)
...
s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
for data in [b'Alice', b'Bob', b'Sarah']:
	# 发送数据
...
## 服务器和客户端程序，必须运行在两个shell窗口中，否则无法运行。且服务器程序需要先运行。
