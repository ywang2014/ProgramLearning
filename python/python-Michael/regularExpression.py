...
# '\d\d\d' 可以匹配：三位数(123) '\w\w\d' 可以匹配：'py3'等
# .可以匹配任意字符： 'py.' 可以匹配：'pyc' 'py3' 'py!'等
...
# Re Python提供re模块，包含所有正则表达式的功能。由于Python的字符串本身也用\转义，所以要特别注意：
s = 'ABC\\-001'	# python的字符串，对应的正则表达式：'ABC|-001'
s = r'ABC\-001' # 使用Python的r前缀，就不用考虑转义的问题了：对应的正则表达式：'ABC|-001'
...
# 举例如下，匹配出数字后面的0：
re.match(r'^(\d+)(0*)$', '102300').group()	# ('102300', '')
# 由于 \d+ 采用贪婪匹配，直接把后面的0全部匹配了，结果 0* 只能匹配空字符串
...
# 编译
	...
	# 如果一个正则表达式要重复使用几千次，可以预编译该正则表达式，重复使用时就不需要编译这个步骤了，直接匹配：
import re
...
## 正则表达式非常强大，你可能需要一本正则表达式的参考书。


# 练习：
	# 请尝试写一个验证Email地址的正则表达式。版本一应该可以验证出类似的Email：bill.gates@microsoft.com
r'^(\w+)@(\w+).com$'
reg = r'^(\w*\d*.*)@(gmail|microsoft).(com)$'
v1 = re.compile(r'^(\w+(?:.\w+)?)@(\w+).(\w+)$') 
v2 = re.compile(r'^(<\w+\s\w+>)\s+(\w+(?:.\w+)?)@(\w+).(\w+)$')
