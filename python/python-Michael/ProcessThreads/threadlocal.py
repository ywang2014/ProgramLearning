...
def do_task_2(std):
	do_subtask_2(std)
	do_subtask_2(std)
# 每个函数一层一层调用都这么传参数??? 全局变量不行，每个线程处理不同的Student对象，不能共享
...
# ThreadLocal 可以实现，不用查找 dict
import threading

# 创建全局ThreadLocal对象
local_school = threading.local()	# ThreadLocal对象
...
## 这样一个线程都有自己独立的资源，它的所有调用到的处理函数都可以非常方便地访问这些资源
