阳历：常用的时间记录法：365/366天每年，12个月，闰年2月29天，小月30天，大月31天。

阴历：中国传统历法，规律不强，通过天文观测推算的，符合24节气
...
每个数据代表一年，从阳历1900.1.31日起，为第一个数据年的开始，即阳历1900.1.31＝阴历0.1.1
150个数据可推150年的阴历，因此目前最大只能推算到2049年
...
2012年10月 (12)
最新评论
1. Re:中断描述符表（Interrupt Descriptor Table，IDT)
博主你好，请问你截图的那个“中断向量表” 是来自哪一本书的呢？
--StrikeW
2. Re:__try，__except，__finally，__leave异常模型机制
楼主分析的甚是详细，赞一个
--丹京
3. Re:PE文件详解一：M_DOS头部IMAGE_DOS_HEADER STRUCT结构
好文章
--那年_lin
4. Re:阳历转阴历算法概述
属相应该是按照农历来的吧，程序中是按照阳历来。
--Cfan1236
阅读排行榜
1. MySql表结构修改详解(11436)
2. 阳历转阴历算法概述(5673)
3. VC++单文档分割窗口、在视图中添加控件，并响应该控件的事件(3117)
4. C++ 函数后加const(2921)
5. GetOpenFileName(2257)
评论排行榜
1. 阳历转阴历算法概述(2)
2. __try，__except，__finally，__leave异常模型机制(1)
3. C语言-文件操作详解(1)
4. PE文件详解一：M_DOS头部IMAGE_DOS_HEADER STRUCT结构(1)
5. an error has occurred while trying to access the log file. logging may not function properly(1)
推荐排行榜
1. PE文件到内存的映射(2)
2. C语言中不安全的函数以及解决方案(1)
3. C语言-文件操作详解(1)
4. PE文件结构体总汇(1)
5. PE文件详解九：资源结构体(1)
阳历转阴历算法概述
