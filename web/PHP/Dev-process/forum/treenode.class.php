...
class Treenode
{
	...
	public function __construct($postid, $title, $author, $post_time, $children, $expand, $depth, $expanded, $sublist)
	{
		...
		{
			$conn = db_connect();
			$sql = "select * from forums where parent = '$postid' order by post_time";
	...
	function display($row, $sublist = false)
	{
		if ($this->m_depth > -1)
		{
			...
			{
				echo "<a href = \"forum.php?collapse=".$this->m_postid."#".$this->m_postid."\" >".
						"<img src = \"images/minus.gif\" valign = \"bottom\" height = \"22\" width = \"22\" alt = \"Collapse Thread\" ".
					"border = \"0\" /> </a> \n";
			}
			else if (!$sublist && $this->m_children)
			{
				echo "<a href = \"forum.php?expand=".$this->m_postid."#".$this->m_postid."\" >".
			...
			echo "<a name = \"".$this->m_postid."\"> <a href = \"view_forum.php?postid=".$this->m_postid."\">".
