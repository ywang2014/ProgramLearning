...
$not_login_arr = array('login', 'act_login', 'register', 'act_edit_password', 'get_password', 'send_pwd_email',
						'password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 
						'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email',
...
if (empty($_SESSION['user_id']))
{
	if (! in_array($action, $not_login_arr))
	{
		...
		{
			/* 如果需要登录，并显示页面操作，记录当前操作，用于登录后的跳转到相应的操作
			if ($action == 'login')
			{
...
{
	assign_template();
	$position = assign_ur_here(0, $_LANG['usr_center']);
	...
	// 是否显示积分兑换
	if (!empty($_CFG['points_rule']) && unserialize($_CFG['points_rul']))
	{
...
if ($action == 'default')
{
	...
	{
		$smarty->assign('rank_name', sprintf($_LANG['next_level'], $rank['rank_name']));
...
elseif ($action == 'act_register')	// 注册会员的处理
{
	// 增加是否关闭注册
	...
	else
	{
		...
		$other['man'] = isset($_POST['extend_field1']) ? $_POST['extend_field1'] : '';
